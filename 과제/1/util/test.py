...
class SQLClient:
    ...
    def sel_all_col(self, con, query, qparse):
        try:
            results = con.execute(query)
            results = results.fetchall() 
            if qparse.output_columns[0] == '*':
...
class Window(QMainWindow):
    ...
    def update(self, con, query, qparse):
        try:
            ...
            self.ui.textBrowser.setText(f"{query} 실행 \n'{results}'건 업데이트 완료")
    ...
    def commit(self):
        ...
        if self.con:
            try :
                ...
                self.con.close()
                self.con = None
                self.ui.textBrowser.setText('Commit 성공')
    ...
    def rollback(self):
        ...
        if self.con:
            try:   
                ...
                self.con.close()
                self.con = None
                self.ui.textBrowser.setText('Rollback 성공')
